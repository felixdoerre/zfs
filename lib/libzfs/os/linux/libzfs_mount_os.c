...
void
zfs_adjust_mount_option_string(zfs_handle_t *zhp, const char *mntpoint,
    char *mntopts, char *mtabopt)
{
...
do_mount(zfs_handle_t *zhp, const char *mntpt, char *opts, int flags)
{
	...
	if (! libzfs_envvar_is_set("ZFS_MOUNT_HELPER")) {
		char badopt[MNT_LINE_MAX] = {0}, mtabopt[MNT_LINE_MAX] = {};
		unsigned long mntflags = flags, zfsflags;
		char *copy = strdup(opts);
		int parse_res = zfs_parse_mount_options(copy, &mntflags,
		    &zfsflags, 0, badopt, mtabopt);
		if (parse_res) {
			return (ENXIO);
		}
		free(copy);
		...
		zfs_adjust_mount_option_string(zhp, mntpt, myopts, mtabopt);
		int error = mount(src, mntpt, MNTTYPE_ZFS,
		    mntflags, myopts);
		if (error) {
			return (error);
		}
